...
### Phase 30: the Feed copy pass

Display only. Nothing about signals, scoring, impact values, ingestion or stored data changed; stored text is never rewritten. Every word on a card now comes from one module, `lib/feed/card-copy.ts`, and the four surfaces that show signals — the Feed, the profile's Signals list, the landing page's "Why it moved" and Home's desktop rail — render through it, so they read as one publication. The rail previously showed stored headlines raw, so a sigma headline could still appear there; it re-renders from the payload like everything else now.

**The anatomy.** A card is a **label** (the outlet, small, above an article's title), a **headline** (the Engine's sentence, or the publisher's title linked to the piece), one plain **line** that says what it was and how far it moved the score, and an **attribution** (where it came from). The coloured direction indicator in the header carries direction and nothing else.

**The seven rules**, held by `lib/feed/card-copy.test.ts` over every card type for every kind of person on the board:
1. **One voice.** The same card types render the same way on all four surfaces.
2. **No template inside a template.** A stored Engine narrative of the shape *X's momentum climbed on "…"* (137 of the last week's 236) is **un-nested at display**: the quoted signal, re-rendered, is the headline and the move goes in the line. The stored sentence is kept in `text`. No rendered headline ever matches the nested shape or quotes another card's sentence.
3. **Name the company.** A company-news metric says *Tesla is in the news more than usual*, never *X's company*. The name comes from the person's Finnhub mapping (the ticker) through `lib/people/company.ts`; `lib/people/company.db.test.ts` reads the seeded mappings and fails if an active one has no name. The comparison is to the company's own pace, *its* not *their*; the expand says *Tesla's usual pace*.
4. **Name the outlet, never the wire.** An article card shows the publication (`outletName`: the known name for its domain, else the feed's own name cleaned of section and tagline, else the domain) and links to the piece. `SOURCE_NOUNS` says what every other source is called: News coverage, Company news, Company filings, Game data, YouTube, YouTube comments, YouTube Trending, Twitch. "RSS", "signal read", "Finnhub", "via" and "API-Sports" never reach a reader; the test sweeps every rendered string.
5. **Words fit the person.** `METRIC_VOICE` gained a category dimension (`categoryGroup`): creators and musicians share the creator nouns; executives, athletes, the founder and anyone uncategorised get the general voice. `viral_moment_rate` (off since 2026-09-25, its 868 rows still render) has a general voice of stories and coverage, so Larry Page's row reads *Stories about Larry Page are spreading unusually fast*; a creator-platform metric (`audience: "creator"`) rendered for anyone outside the group speaks `NEUTRAL_VOICE`, which has no nouns at all, and the db test holds that such metrics are declared on creator and musician mappings only.
6. **Zero-impact items are not cards.** `showsAsCard`: nothing the Engine has not read, and nothing whose move prints as zero at the one decimal a card shows (`CARD_HIDDEN_BELOW = 0.05`). Over the seven days to 2026-09-25 that is 778 of 2,445 processed signals at exactly 0.00 (comment digests, expired items, neutral reads) and 17 more between 0.01 and 0.04, which would otherwise have read "+0.0". They stay in detail panels and on the profile's list. The Feed's page cursor is taken from the last **row** before hiding, so paging is unaffected.
7. **The person is named once in the body.** Our headline or the line, never both: *Tesla is in the news more than usual · Company news · Musk +0.8*, but *MrBeast's channel is pulling views faster than usual · YouTube · +0.9*. An article's title is the publisher's text and does not count; its line names the person: *Read as positive for Zuckerberg · +0.3*. The surname is the last word of the display name.

**What ingestion keeps writing.** The stored `signals.headline` is untouched in kind: `metricSignal()` now passes the person's category so a creator's YouTube or Twitch reading keeps its creator nouns word for word, and a company metric with no company to hand (the writer has only the person) uses the Phase 21+ lines (`withoutCompany`, *X's company is in the news more than usual*) exactly as before. Every reader-facing surface re-renders from the payload with the category and the company, so the stored line is never what a reader meets. `lib/ingest/privacy.db.test.ts` names the two new server-only readers of `raw_payload` (`lib/feed/enrich.ts`, `lib/home/board.ts`) in its allow-list.

**What the server adds.** `feed_entries()` is unchanged (no migration). `lib/feed/enrich.ts` reads, for exactly the signals on a page, the outlet, domain, link and payload kind, and a comment digest's lean, video title and sample count, through the service-role client, and projects them (`projectSignalDetail`) before anything leaves the server: a digest's sampled comment texts are never read into the projection. The company lookup is one read per request, shared by the Feed, the profile and the rail. A comment digest re-renders from its shape (*Comments under "Can We Build an Entire Village?" are mixed.*); a game result, a trending appearance and a filing render as stored.

**Verified.** `card-copy.test.ts` (the seven rules over 600-plus rendered cards, the outlet and noun tables, the projection), `metric-language.test.ts` (every metric × every category × every band carries no creator noun outside the group; company naming and possessive), `feed-model.test.ts` (un-nesting, cursor-before-hiding, company from context), `company.db.test.ts`; typecheck, lint, suite and build green. Screenshots at 375 and 1280 of a fixture Feed with creator, executive and athlete cards, a metric card's detail open and a narrative's "What the Engine saw" open; the rendered DOM was swept for the banned words (none). The fixture page and its temporary gate entry were deleted before the commit.

### Phase 31: matching and narrative fixes, designed and replayed on a branch (not shipped)

Five changes to what enters the pipeline and how the model describes it, built on the branch `phase31-matching-narrative` behind **one switch**, `SIGNAL_QUALITY_ENABLED` (exactly `"true"`, like the cron flags; ships unset). Off, a tick and a poll are byte-for-byte what they were: `lib/engine/signal-quality.test.ts` runs the same seed with the switch off and on, and `lib/engine/sentiment/prompts.test.ts` pins the production prompt's hash. Nothing here moves a score until the operator turns the switch on, after the [09-28 weight-wave read](#reading-the-09-28-wave). The branch is not deployed (`vercel.json` `git.deploymentEnabled` names it false).

**1. The wrong person.** Three rules in `lib/ingest/disambiguation.ts`, all configuration on the person's `rss` mapping: `exclude_unless_named` (a term that marks a namesake's story *unless the subject is named alongside it*: `"David Ellison"`, `"Skydance"`, `"Paramount"` for Larry Ellison, `"Page Auto"` for Larry Page), `namesake_guard` (an item from an unknown publisher, the tier-5 floor, must name the subject in its headline), `aliases` (what else counts as naming: `"Larry"` inside his own feed, `"Nvidia CEO"` for Jensen Huang), and one rule that needs no configuration, the **obituary guard** (a funeral notice's vocabulary in the headline, or a funeral home in the outlet or domain). Replayed on every stored article: the guard hits exactly three rows, all namesakes' notices, and no legitimate item. The rules stop all five wrong items of the 105-signal sample (Variety and Deadline on David Ellison, the two obituaries, Page Auto Group) and, over the fortnight, the ten David Ellison stories scored for Larry at +4.32 of absolute impact. Collateral: "Paramount unless named" also refuses Paramount deal coverage that never names him (the WSJ concession story, +0.22; the settlement pieces, 0.00), by decision; the namesake guard refuses 18 unnamed tier-5 Ellison items (the largest: "John Oliver Roasts MAGA Nepo Baby", −0.25, about David), 7 Page (all wealth lists, 0.00) and 7 Dell, two of which are the "Zuckerberg overtakes Dell" stories (−0.30, −0.15) that the salience label handles. **Decided 2026-09-26: Dell gets no namesake guard.** The obituary guard covers his one real namesake risk (the Aftercare notices) and the salience label handles the rest; Ellison and Page keep their rules as designed. The full-name form of the rule the request named ("require the person's full name in the headline") was replayed too and is **rejected**: over 14 days it would refuse 229 tier-5 items carrying 19.5 points of absolute impact, most of them company stories ("Nvidia CEO says…", "Meta jumps 11%", "Tesla eyes supply partners"), against 1.08 points of wrong-person impact.

**2. Salience by information, not prominence.** The version-2 prompt asks, per signal, what the story *says about this person's trajectory*: `relevant` (information about them even as a secondary party: the donor named, the founder whose company is in the news, and anything the person **did**: a deal, a donation, a statement, a sale, a pledge, a filing, whatever dollar figure is attached), `wealth_ranking` (decided 2026-09-26: the only information about the person is that their net worth or their place in a rich list changed, and the change is a stock price moving), `incidental` (named without anything said: an attendee list, a passing comparison), `unrelated` (a namesake). The Signals force multiplies by 1.0 / 0.3 / 0.3 / 0 (`config.signalQuality.salienceMultipliers`); `wealth_ranking` sits at incidental's weight **until counsel rules** on where such stories stand against the no-security-price rule. The 34 passing mentions of the sample reclassified by hand under this definition: **18 relevant, 16 incidental**; Dell's two overtaking stories and all five of Brin's Prop. 40 and ranking items come out relevant, as required. Signed impact of the 39 passing-or-wrong items +1.40 → +0.49, absolute 4.30 → 2.79; by person, Ellison 1.01 → 0.19 (the three wrong items to zero), Dell 0.71 → 0.45, Mahomes 0.24 → 0.07, Brin unchanged at 0.62. Under the fourth label Dell's two overtaking stories and Brin's two "passed by Zuckerberg" items move from relevant to `wealth_ranking`, at the same 0.3, so the totals above hold and only Dell (0.71 → 0.14) and Brin (0.62 → 0.33) fall further. The model's own labels have not been replayed: this environment holds no model key (see the open item).

**The wealth-ranking replay, 14 days to 2026-09-26, five executives, every article signal hand-labelled.** A story is `wealth_ranking` when its only information about the person is a rich-list or net-worth change driven by shares (an overtaking, "fortune jumps", "falls to fourth as the stock tanks", a Forbes list); a story in which the person acted stays relevant (Bezos putting $30 billion of his fortune into Blue Origin, Brin's $100 million against the wealth tax, Page leaving California, Ellison pledging Oracle shares). Absolute and signed impact of every article, before and after the 0.3 weight on the wealth-ranking ones:

| Person | Wealth-ranking stories | Absolute impact | Signed impact |
|---|---|---|---|
| Michael Dell | 17 of 48 | 2.60 → 1.56 | −0.84 → +0.15 |
| Sergey Brin | 6 of 54 | 1.50 → 1.21 | −0.43 → −0.14 |
| Larry Page | 8 of 41 | 0.08 → 0.08 | −0.08 → −0.08 |
| Jeff Bezos | 9 of 70 | 4.39 → 4.06 | +3.58 → +3.32 |
| Larry Ellison | 3 of 68 | 9.92 → 9.92 | +6.88 → +6.88 |

Dell is the finding: nearly everything that moved him down in the fortnight was being overtaken (−0.90 Forbes, −0.30, −0.15, −0.09 as Dell stock fell 6 %), and with those at 0.3 his fortnight turns from −0.84 to +0.15. Page's and Ellison's wealth stories all scored 0.00, so the label changes their lists and nothing else. The shares here are lower than the regex count reported earlier (Brin 39 %, Page 34 %) because that count swept up the stories in which they acted, Brin's tax fight above all.

**3. One event, one signal.** `lib/engine/stories.ts`, at scoring: a new article that tells the same story as one the person's tick already scored inside 48 hours (or as a stronger copy in the same tick) is a *confirmation*: a fifth of its own impact, never more than 0.15 points, never the full impact again; the first copy keeps what it earned. "Same story" is the ingestion rule (Dice ≥ 0.4) or, from Dice 0.25, a shared **anchor**: a number with its unit ("9 billion", "1299"), or a phrase of two consecutive content words that is not the person's everyday vocabulary (a phrase in more than three of their recent headlines, or two everyday words, anchors nothing: "chiefs qb", "kansas city"). Replayed over the 982 articles of the week to 09-25 (`npm run replay:stories`): 85 confirmations, absolute impact 161.0 → 139.7 (13.2 % removed). Of the four named cases, Zuckerberg's $9 billion (−1.11 twice → −1.11 and −0.15), Martha Stewart (+0.38, +0.48 → +0.38, +0.10) and Kendrick's tour (+0.38, +0.48 → +0.38, +0.10) are caught; **the Huang interview is not**, and cannot be by any lexical rule: its five takes quote five different sentences and share no phrase and no number. It is caught only when the takes arrive in one chunk, which the prompt already covers. Read one by one, about 51 of the 85 are the same event, 13 are the same thread told twice (a response to remarks, the next angle of a launch), and 21 are different stories that share a phrase, 15 of them Mahomes's (his co-stars: "Tom Brady", "Josh Allen"). The false confirmations cost 5.4 points of legitimate impact over the week against 16 points of duplicate impact removed; raising the anchor threshold to 0.30 would cut the false ones to 10 and also lose Kendrick's tour, Dell's overtaking pair and eleven other true matches. **Decided 2026-09-26: 0.25 stays**; a per-person stoplist of frequently co-occurring names (teammates, opponents) is the open item for Mahomes's false confirmations, tuned after shipping. **Clustering changes nothing in `person_signal_volume()`**: a confirmed copy is still a stored, processed row and still counts.

**4. Stale on arrival.** The news connector refuses an item published more than `freshnessMaxAgeHours` (168 h) before the poll, with a note on the poll row like the undated rule. These items already scored at exactly zero (the freshness curve), so no score changes; what changes is the person's recent lists and, for items under 16 days old, the volume series. Counted: in the week of 09-14 (each subject's first week on the feed) 44 of Adin Ross's 47 stored articles were stale on arrival, Brin 34 of 42, MrBeast 31 of 55, Drake 29 of 98, Kendrick 29 of 44, Page 24 of 34, Bezos 22 of 41; in the week of 09-21, Adin 9 of 12, Dell 8 of 21, everyone else 0 to 4. Only 20 stale rows sat inside the volume window (Bezos 6, Dell 4, Huang 3, Kendrick 3), none inside the five complete days the weight will read on 09-28.

**5. The analyst's note.** The version-2 prompt asks for one sentence written for a reader who follows the person (what happened, why it matters, the person named at most once, no weighing vocabulary, no multiples or baselines) and a declared `narrative_direction`; `checkNarrative()` in `lib/engine/narratives.ts` replaces a sentence with the template when its direction disagrees with the Signals force (or calls a half-point move flat) or when it speaks in the Engine's voice ("signal", "noise", "digest", "priced in", "2.5x", σ), and the post-tick step logs each replacement. The version-1 prompt is untouched. `scripts/replay-prompts.ts` (`npm run replay:prompts`, then again with `--v1`) rebuilds recent LLM narratives' chunks from `narrative_signals`, asks the model and prints the per-signal label agreement with the stored labels, the salience distribution with every non-relevant headline, and every old and new sentence with the direction check; **it has not been run**. What it needs: the model key the provider reads (`ANTHROPIC_API_KEY`) and the three routing variables production sets, `LLM_PROVIDER`, `LLM_MODEL` and `LLM_MODEL_SENTIMENT`, at production's values (the sentiment route reads `LLM_MODEL_SENTIMENT`, then `LLM_MODEL`), so the comparison is of prompts and not of models; and one of three database accesses, tried in this order. (a) `REPLAY_DATABASE_URL`, a direct Postgres connection as a **read-only role** (the script also sets its session read-only). The six tables it reads (`narratives`, `narrative_signals`, `signals`, `people`, `data_sources`, `person_memory`) carry `SELECT` policies for `authenticated` and nothing for a new role, so the role has to be made a member of `authenticated` to see rows: `create role replay_reader login password '…' inherit; grant authenticated to replay_reader;` — one role, no table DDL, no migration; it inherits `authenticated`'s grants, whose write policies all key on `auth.uid()`, which a direct connection never has. (b) `REPLAY_USER_EMAIL` and `REPLAY_USER_PASSWORD` with `NEXT_PUBLIC_SUPABASE_URL` and `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY`: an ordinary account signed in through the publishable key reads the same six tables under the same policies, with no privileged key and no role at all. (c) `SUPABASE_URL` with `SUPABASE_SERVICE_ROLE_KEY`, the last resort. So yes, a read-only role suffices, and so does an ordinary account. The sandbox this was built in reaches `api.anthropic.com` and neither the project's REST host nor its database host (both refused by the network policy), so the environment needs `smtqjpwjbrvzzoosccjd.supabase.co` on 443 for (b) or (c), or the database host (or the session pooler) on 5432 for (a).

**Effect on the volume weight.** Only the ingestion refusals change `person_signal_volume()`; salience and clustering happen at scoring and leave the rows. Replayed on the five complete days (09-21 to 09-25) the weight will read on 09-28: Ellison's series 6, 6, 5, 7, 4 → 2, 2, 1, 1, 2 (mean 5.6 → 1.6, weight 0.71 → 2.00, the ceiling), Dell 2, 2, 1, 4, 2 → 2, 1, 1, 2, 0 (2.2 → 1.2, 1.82 → 2.00), Page 0.8 → 0.4 (already at the ceiling); every other person is unchanged to the row, because the stale rows they carry all fall outside those days. Ellison's is the one to watch: two thirds of his stored volume was David Ellison and Paramount, and his weight will sit on the ceiling once they are gone. **His volume weight moves 0.71 → 2.00 when the switch flips**, so every event signal of his counts 2.8× what it would have counted with the rows kept, and 2× what it counts today (the weight is exactly 1 until the baseline is sufficient on 09-28). In points: a tier-1 story he is named in at confidence 0.8 carries 1.5 × 1.5 × 0.8 = 1.8 before the weight, 1.28 at 0.71, **3.6 at 2.00**; a tier-5 story at the same confidence 0.36 → 0.72. The per-tick brake of 10 and the quadrature sum still apply, and his cleaner feed runs about 1.6 events a day, so the amplification lands on a few real stories rather than many. Five other people already sit on the same ceiling.

**Shipping plan.** One isolated change, after the 09-28 read: set `SIGNAL_QUALITY_ENABLED=true` in Vercel (both halves at once), and in the same hour the configuration rows the rules read: Ellison's `exclude_unless_named: ["David Ellison", "Skydance", "Paramount"]`, `namesake_guard: true`, `aliases: ["Larry", "Oracle founder", "Oracle co-founder"]`; Page's `exclude_unless_named: ["Page Auto"]` and `namesake_guard: true`; **nothing for Dell** (decided: no guard). Nothing else. Expect, from the first tick after the flip: Ellison's volume weight at the ceiling (0.71 → 2.00, above) and his stories counted 2× today's, a quieter Ellison feed (about 1.6 events a day instead of 5.6), Dell's downward drift from rankings damped to 0.3, and template narratives in place of any note the direction or voice check refuses (logged by the post-tick step). Before the flip: run `replay-prompts` twice, version 2 and `--v1`, and read the label shift against the model's own variance; if the prompt moves labels by more than the variance, that is a generation change to weigh, not a display change. Reversal is the switch back to unset. Configuration rows are inert while it is off.
...
- **Phase 31, before the switch is turned on**: run `npm run replay:prompts` once with version 2 and once with `--v1` (access and variables in the Phase 31 section), and compare the per-signal label agreement of each against the stored labels: the difference between the two runs is the prompt's effect on scoring, the `--v1` run alone is the model's own variance. Report the salience distribution and every direction-check replacement.
- **Phase 31, `wealth_ranking` weight**: 0.3 (incidental's) until counsel rules on where a rich-list change driven by a stock price sits against the no-security-price rule; then either keep it, lower it, or exclude the label from scoring altogether.
- **Phase 31, a per-person stoplist for story anchors**: frequently co-occurring names (teammates, opponents, rivals: "Tom Brady", "Josh Allen", "Travis Kelce" for Patrick Mahomes) as a configured list on the mapping, excluded from phrase anchors, to cut the false confirmations that were 15 of the 21 in the replay. Tune after shipping, on the confirmations the force records.
...
- **Phase 31 (branch `phase31-matching-narrative`, not shipped)**: matching and narrative fixes behind one switch, `SIGNAL_QUALITY_ENABLED` — name-conditional exclusions, a namesake guard for unknown publishers and an obituary guard at ingestion; stale-on-arrival refusal; a four-way salience label (relevant, wealth_ranking, incidental, unrelated) multiplying the Signals force; story confirmation at scoring (a copy of a scored story contributes a bounded fifth, never full impact again); the analyst's note with a direction and voice check; two replay scripts. Replayed against the week to 09-25, not run in production.
- **Phase 30**: the Feed copy pass, display only. One renderer (`lib/feed/card-copy.ts`) for the Feed, the profile's Signals list, the landing page and the Home rail: outlets named and linked, companies named from the Finnhub mapping, nouns chosen by the person's category, template narratives un-nested at display, zero-impact items kept out of the Feed, the person named once per card. No migration; the server adds outlet, link and a digest's shape per page and never the comment texts.
