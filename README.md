...
A season is scheduled, periodic and outcome-bearing, which is a shape nothing else here has. **Game results are events**: discrete, dated, and already in language the sentiment path reads, one signal per game keyed on the game id. **One metric is registered** — `game_passing_yards` — intended to be sampled *once per game* rather than once per poll, so `samples` in the baseline counts performances and not hours. (As of 2026-09-17 the connector does not yet produce it: see *What the first live response taught* below.)
...
**What the first live response taught (2026-09-17).** The Pro key resolved player 1197 and the endpoint returned one entry, and the statistic was not under any dotted path because the American Football host does not key statistics at all: it returns named **groups** of name/value pairs — `teams[0].groups[{name: "Passing", statistics: [{name: "yards", value: "3,587"}, …]}]` — with `yards` repeated under Rushing and Receiving. Three consequences, all in `lib/connectors/apisports.ts`: a statistic is addressed by **group and name** (`config.passing_yards_stat`, default `{group: "Passing", name: "yards"}`, the dotted `passing_yards_keys` kept as a fallback for a keyed shape); values are **strings with thousands separators**, parsed by `parseStatValue` against exactly that grammar and refused out loud otherwise (`"3,587"` is 3587 and can never come back as 3; an unparseable value fails the poll naming the value, and a missing one names the groups and statistics that are present); and the figure is the **season cumulative total** — the shape this connector deliberately refuses to register as a metric — so it is snapshotted raw, once per change, under `season_passing_yards` with no observation and no signal. `game_passing_yards` is not produced until its source is settled: the per-game statistics endpoint, or a difference of consecutive totals.
