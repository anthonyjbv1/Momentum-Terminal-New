...
- **Phase 31, before the switch is turned on**: run `npm run replay:prompts` (needs `SUPABASE_SERVICE_ROLE_KEY` and a model key) once with version 2 and once with `--v1`, and compare the per-signal label agreement of each against the stored labels: the difference between the two runs is the prompt's effect on scoring, the `--v1` run alone is the model's own variance. Decide Dell's namesake guard (it refuses the two overtaking stories the salience rule wants) and whether wealth-ranking stories, 34 to 40 % of Dell's, Brin's and Page's articles, belong under the no-security-price rule.
