...
| `market_tier_settings` | One row per tier (`public_figure`, `private_individual`): depth, decay half-life, premium cap, minimum hold, max order share of depth, aggregate exposure cap, the breakers, shorting allowed, alert on halt. A NULL depth is the tier's off switch. Readable signed in; changed by migration (Phase 29) |
...
A **NULL depth is the tier's off switch**: a flat market — Phase 27 pricing exactly, premium 0, no impact, no `premium_history` rows — with no deploy; switching a tier off with inventory standing writes one `reset` row per person and returns the inventory to zero. The legacy suites run on the flat market and pass unchanged.
